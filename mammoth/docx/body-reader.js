  ...
  function readRunProperties(element) {
    return readRunStyle(element).map(function (style) {
      ...
      return {
        ...
        fontSize: fontSize,
        background: element.firstOrEmpty("w:shd").attributes["w:fill"],
  ...
  var xmlElementReaders = {
    ...
    "w:pPr": function (element) {
      return readParagraphStyle(element).map(function (style) {
        let _stls = styles.findParagraphStyleById(style.styleId);

        let rPrs;
        if (_stls && _stls.props && _stls.props.pPr) {
          element.children = [...element.children, ..._stls.props.pPr.children];

          if (_stls.props.rPr) {
            // let __rpr = element.children.find((e) => e.name == "w:rPr");

            // __rpr
            //   ? __rpr.children.push(..._stls.props.rPr.children)
            //   : element.children.push(_stls.props.rPr);
            rPrs = _stls.props.rPr;
          }
        }
        let ___ = element.children.find((e) => e.name == "w:rPr");
        // console.log(rPrs, "e", ___);
        let _dd = rPrs ? readRunProperties(rPrs) : {};
        // console.log(_dd);
        //

        let rprStl = { ..._dd.value };
        delete rprStl.type;
        //console.log(numbering, "tt");

        let spacing = element.firstOrEmpty("w:spacing").attributes;
        let spacingData = {
          after: Number(spacing["w:after"]) / 20,
          before: Number(spacing["w:before"]) / 20,
          line: Number(spacing["w:line"]) / 20,
        };
        return {
...
function readNumberingProperties(styleId, element, numbering) {
  ...
  if (level === undefined || numId === undefined) {
    ...
    //console.log(numbering.findLevel(numId, level), "Numberings....");
    return { ...numbering.findLevel(numId, level), numId };
  }
}
